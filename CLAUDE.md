...
- 상단 메뉴는 `Home` `Scenes`
- 장면 카드 이미지는 `public/thumbs/<slug>.webp` — 실제 플레이 화면 캡처다. 장면을 바꾸면 서버를 띄우고
  `npm run thumbs` (옆 프로젝트 klead 의 playwright 를 빌려 쓴다 · `PLAYWRIGHT_DIR`)
- 기울기는 **https 에서만** 동작한다(브라우저 정책). 플레이어가 켤 때 https · 허용 · 수신 여부를 안내한다
